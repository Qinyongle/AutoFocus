...
public:
    ...
    std::function<void(bool)> _replaystate = nullptr;
...
private:
    ...
    void PacketSend(AVPacket *);
    void FileSendTimerThread();

private:
    std::string m_url;
    std::unique_ptr<std::thread> m_getFrameThread = nullptr;
//    std::unique_ptr<std::thread> m_fileSendTimer = nullptr;
    ...
    bool m_fileSendSwitch = false;
    bool m_fileSendTimerSwitch = false;
