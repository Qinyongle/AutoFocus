...
//        m_saveVideos->SetAVFormat(iFormat, data);
    ...
    m_getFrames->_bindAVPacketSendPtr = [this] (AVPacket *data) {
        m_playBackPlayLable ? m_qtFfmpegDecoder->GetAVPacket(data) : void();
...
void VideoRealTimeGet::GetPicCmd()
{
    m_qtFfmpegDecoder->GetPicCmd();
}

//先录制--在播放
void VideoRealTimeGet::SetSavePath(QString path, QString name)
{
//    m_saveVideos->SetFilePath(path.toStdString(),
//                              name.toStdString());
}
